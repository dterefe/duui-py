...
def _model_name(doc: TextImagerRequest) -> str:
    ...
    if lang == "x-unspecified":
        lang = "xx"
    variant = str(
        params.get("model_variant") or params.get("spacy_model_size") or "efficiency"
    )
    ...
    if lang not in SPACY_MODELS[variant]:
        unprocessable(
            "Unsupported spaCy language for variant.", language=lang, variant=variant
        )
...
def _morph_details(token: Any) -> dict[str, object]:
    ...
    return {
        legacy_key: raw[source_key]
        for source_key, legacy_key in MORPH_KEYS.items()
...
def _make_doc(nlp: Any, doc: TextImagerRequest) -> tuple[Any, bool]:
    is_pretokenized = bool(
        doc.tokens and doc.spaces and (len(doc.tokens) == len(doc.spaces))
    )
...
class SpacyLegacyAnnotator(DuuiAnnotator[TextImagerRequest, dict[str, object]]):
    config = AnnotatorConfig(
        meta=AnnotatorMeta(
            meta={"source": "TTLab-UIMA/textimager-uima-spacy legacy Lua migration"}
        ),
    ...
    @telemetry.timed("spacy_legacy_processing_ms", annotator="spacy-legacy")
    async def process(self, doc: TextImagerRequest) -> dict[str, object]:
        ...
        await telemetry.trace(
            "spaCy legacy processing started",
            model=model_name,
            write_types=sorted(write_types),
        )
        ...
        await telemetry.count(
            "spacy_legacy_annotations",
            len(sentences) + len(tokens) + len(dependencies) + len(entities),
            model=model_name,
        )
