...
class LinkerPathsFactory:
    def __init__(self):
        self.factory = {
            ...
            "gbif_fuseki": GbifLinkerPaths,
            "biofid_fuseki": GbifLinkerPaths,
...
class CandidateGenerator:
    ...
    def __call__(
        ...
        batch_mention_alias_scores = []
        all_neighbor_aliases = {}
        for neighbors, distances in zip(batch_neighbors, batch_distances):
            ...
            for neighbor_index, distance in zip(neighbors, distances):
                ...
                mention_to_similarity[alias] = 1.0 - distance
                all_neighbor_aliases.setdefault(alias, None)

            batch_mention_alias_scores.append(mention_to_similarity)

        if all_neighbor_aliases:
            mentions_to_concepts = self.kb.get_cuis_from_aliases(all_neighbor_aliases.keys())
        else:
            mentions_to_concepts = {}
        batch_mention_candidates = []
        for mention_to_similarity in batch_mention_alias_scores:
