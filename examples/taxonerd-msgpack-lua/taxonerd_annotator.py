...
# ---------------------------------------------------------------------------
# Model / linker / strategy lookup tables
# ---------------------------------------------------------------------------
TAXONERD_MODELS = {
...
# ---------------------------------------------------------------------------
# Module-level caches & thread-local state
# ---------------------------------------------------------------------------
...
class TaxoNERDAnnotator(DuuiAnnotator[V1RequestEnvelope, object]):
    """DUUI annotator that wraps the TaxoNERD taxonomic NER + linking pipeline.

    All domain helpers live as private static methods on this class so that
    the ``process()`` entry point is the single focal point for readers.
    """
    ...
    # ==================================================================
    # Parameter resolution helpers (static – no self needed)
    # ==================================================================
    ...
    def _aliased_types(
        ...
        out: list[str] = []
        for item in TaxoNERDAnnotator._csv(value, default):
    ...
    # ==================================================================
    # Offset / link extraction helpers
    # ==================================================================
    ...
    def _links_from_ent(ent: Any) -> list[dict[str, object]]:
        try:
            return TaxoNERDAnnotator._links(ent._.kb_ents)
        except Exception:
            return []
    ...
    def _taxon_from_row(row: dict[str, object], text: str) -> Taxon:
        ...
        offsets = TaxoNERDAnnotator._offsets(row.get("offsets"), text, mention)
        if offsets is None:
            bad_gateway("TaxoNERD returned a row without usable offsets.", row=row)
        begin, end = offsets
        links = TaxoNERDAnnotator._links(row.get("entity"))
    ...
    def _taxon_from_legacy_row(row: list[object], text: str) -> Taxon:
        ...
        mention = str(row[1])
        links = TaxoNERDAnnotator._links(row[2]) if len(row) > 2 else []
        identifier = links[0]["id"] if links else None
        link_value = links[0].get("value") if links else None
        link_score = links[0].get("probability") if links else None
        value = text[begin:end] if text and 0 <= begin <= end <= len(text) else mention
    ...
    def _taxon_from_ent(ent: Any, base_offset: int, linker: str | None) -> Taxon | None:
        ...
        if "\n" in ent.text.strip("\n"):
            return None
        links = TaxoNERDAnnotator._links_from_ent(ent)
    ...
    def _load_taxonerd(
        ...
        TaxoNERDAnnotator._require_livb_ner_model(ner, model)
        logger.info(
            "TaxoNERD model loaded: model=%s linker=%s threshold=%.3f prefer_gpu=%s",
            model, linker, threshold, prefer_gpu,
        )
        return ner
    ...
    def _load_taxonerd_ner_only(model: str, exclude: tuple[str, ...], prefer_gpu: bool):
        return TaxoNERDAnnotator._load_taxonerd(
            model, None, 0.0,
            TaxoNERDAnnotator._ner_only_exclude(exclude),
            prefer_gpu,
        )
    ...
    def _configure_taxonerd_fuseki_linker(taxonerd: object, endpoint: str | None, batch_size: int, concurrency: int, timeout: float) -> None:
        ...
        if generator is not None:
            TaxoNERDAnnotator._configure_fuseki_kb(
                getattr(generator, "kb", None),
                endpoint, batch_size, concurrency, timeout,
            )
    ...
    def _load_exact_linker(linker: str):
        try:
            ...
            unavailable(
                "TaxoNERD linker utilities are not installed in this runtime.",
        ...
        try:
            ...
            pass
        logger.info("Exact-match linker loaded: %s", linker)
        return kb
    ...
    def _load_candidate_generator(linker: str):
        try:
            ...
            unavailable(
                "TaxoNERD ANN candidate generator is not installed in this runtime.",
                exception=type(exc).__name__,
            )
        ...
        logger.info("ANN candidate generator loaded: %s", linker)
    ...
    async def _flush_telemetry_queue() -> None:
        try:
            ...
            return
        event_logger = get_configured_event_logger()
        queue = getattr(event_logger, "_queue", None) if event_logger is not None else None
        if queue is not None:
            await queue.join()

    # ==================================================================
    # Exact-linker helpers
    # ==================================================================
    ...
    def _candidate_links_original(
        ...
        with _CANDIDATE_GENERATOR_LOCK:
            ...
            TaxoNERDAnnotator._record_linker_backend_stats(generator)
        return TaxoNERDAnnotator._candidate_links_from_batches(
            mention_strings, batch_candidates, generator, threshold,
        )
    ...
    # ==================================================================
    # Mention batch linker (exact + ANN dispatch)
    # ==================================================================
    ...
    def _link_mention_batch(
        ...
        mention_strings = list(dict.fromkeys(
            TaxoNERDAnnotator._link_key(mention)
            for mention in mention_source
            if TaxoNERDAnnotator._link_key(mention)
        ))
        ...
        for mention_string in mention_strings:
            ...
            cached_value = TaxoNERDAnnotator._link_cache_get(cache_key) if use_cache else None
        ...
        ann_started = time()
        TaxoNERDAnnotator._clear_linker_backend_stats()
        if strategy in {"exact-first-original", "ann-original"}:
            ann = TaxoNERDAnnotator._candidate_links_original(
                ann_pending,
                linker,
                neighbours,
                threshold,
        ...
        backend_stats = TaxoNERDAnnotator._linker_backend_stats()
    ...
    def _taxons_from_doc(spacy_doc: Any, base_offset: int, linker: str | None) -> list[Taxon]:
        return TaxoNERDAnnotator._dedupe_taxons(
            taxon
            for ent in spacy_doc.ents
            for taxon in [TaxoNERDAnnotator._taxon_from_ent(ent, base_offset, linker)]
    ...
    def _taxons_from_mentions(
        ...
        return (
            TaxoNERDAnnotator._dedupe_taxons(
                ...
                for mention in mentions
                for taxon in [
                    TaxoNERDAnnotator._taxon_from_mention(
                        mention,
                        TaxoNERDAnnotator._links(linked.get(TaxoNERDAnnotator._link_key(mention))),
                        linker,
                    )
                ]
                if taxon is not None
    ...
    def _select_spans(
        ...
        for item in items:
            span = TaxoNERDAnnotator._valid_span(item, text_len, accepted)
            if span is None:
                continue
    ...
    def _windows(
        ...
        if not merge_spans:
            ...
            for begin, end, source_type in spans:
                if end - begin > max_chars:
                    out.extend(
                        TaxoNERDAnnotator._split_long_range(
        ...
        for begin, end, source_type in spans:
            ...
            if end - current_begin <= max_chars:
                current_end = end
                current_type = (
                    f"{current_type}+{source_type}"
                    if source_type not in current_type
                    else current_type
                )
                continue
            out.append(
                TextWindow(
                    ...
                    text[current_begin:current_end], current_type,
                )
            )
            current_begin, current_end, current_type = begin, end, source_type
        ...
        for window in out:
            if window.end - window.begin > max_chars:
                split.extend(
                    TaxoNERDAnnotator._split_long_range(
                        text, window.begin, window.end,
                        max_chars, overlap_chars, window.source_type,
                    )
                )
    ...
    def _run_whole_document(
        ...
        logger.info(
            "TaxoNERD whole-document strategy: model=%s linker=%s text_len=%d",
            model, linker, len(text),
        )
        load_started = time()
        taxonerd = TaxoNERDAnnotator._load_taxonerd_ner_only(model, exclude, prefer_gpu)
        load_ms = (time() - load_started) * 1000.0
        logger.debug("TaxoNERD NER-only model loaded in %.1f ms", load_ms)

        ner_started = time()
        doc = taxonerd.ner(text)
        ner_ms = (time() - ner_started) * 1000.0
        logger.debug("TaxoNERD NER completed in %.1f ms", ner_ms)

        mentions_started = time()
        mentions = TaxoNERDAnnotator._mentions_from_doc(doc)
        mentions_ms = (time() - mentions_started) * 1000.0
        logger.debug("TaxoNERD mention extraction: %d mentions in %.1f ms", len(mentions), mentions_ms)

        logger.info(
            "TaxoNERD linking %d mentions with strategy=%s linker=%s",
            len(mentions), linker_strategy, linker,
        )
        ...
        logger.info(
            "TaxoNERD whole-document: %d taxons from %d mentions",
            len(taxons), len(mentions),
        )
    ...
    def _run_legacy_procedure(
        ...
        logger.info(
            "TaxoNERD legacy-procedure: model=%s linker=%s text_len=%d",
            model, linker, len(text),
        )
        ...
        try:
            ...
            logger.error(
                "TaxoNERD legacy-procedure processing failed: %s - %s",
        ...
        taxons = TaxoNERDAnnotator._dedupe_taxons(
            TaxoNERDAnnotator._taxon_from_legacy_row(row, text) for row in rows
        )
        logger.info(
            "TaxoNERD legacy-procedure: %d taxons from %d rows",
            len(taxons), len(rows),
        )
        return StrategyResult(
            taxons,
            windows=1,
            mentions=len(taxons),
            metrics=TaxoNERDAnnotator._taxonerd_linker_metrics(taxonerd),
        )
    ...
    def _run_legacy_compatible_whole_document(
        ...
        logger.info(
            "TaxoNERD legacy-compatible: model=%s linker=%s text_len=%d",
            model, linker, len(text),
        )
        load_started = time()
        taxonerd = TaxoNERDAnnotator._load_taxonerd_legacy_components(model, exclude, prefer_gpu)
        load_ms = (time() - load_started) * 1000.0
        logger.debug("TaxoNERD legacy components loaded in %.1f ms", load_ms)

        ner_started = time()
        doc = taxonerd.ner(text)
        ner_ms = (time() - ner_started) * 1000.0
        logger.debug("TaxoNERD legacy NER completed in %.1f ms", ner_ms)

        mentions_started = time()
        mentions = TaxoNERDAnnotator._mentions_from_doc(doc)
        mentions_ms = (time() - mentions_started) * 1000.0
        logger.debug("TaxoNERD legacy mention extraction: %d mentions in %.1f ms", len(mentions), mentions_ms)

        logger.info(
            "TaxoNERD legacy linking %d mentions with strategy=%s linker=%s",
            len(mentions), linker_strategy, linker,
        )
        taxons, linker_metrics = TaxoNERDAnnotator._taxons_from_mentions(
            mentions,
        ...
        logger.info(
            "TaxoNERD legacy-compatible: %d taxons from %d mentions",
            len(taxons), len(mentions),
        )
        return StrategyResult(
            taxons,
    ...
    def _run_span_windows(
        ...
        logger.info(
            "TaxoNERD span-window: model=%s linker=%s span_types=%s text_len=%d",
            model, linker, list(span_types), len(text),
        )
        spans = TaxoNERDAnnotator._select_spans(fs_items, len(text), span_types)
        if not spans:
            logger.error("TaxoNERD span-window: no matching spans found for types=%s", list(span_types))
            unprocessable(
                "TaxoNERD span-window strategy requires span annotations in the input CAS.",
                span_types=list(span_types),
            )
        logger.debug("TaxoNERD span-window: selected %d spans", len(spans))

        windows = TaxoNERDAnnotator._windows(text, spans, max_window_chars, overlap_chars, merge_spans)
        if not windows:
            logger.error("TaxoNERD span-window: no non-empty windows from %d spans", len(spans))
            unprocessable("TaxoNERD span-window strategy found no non-empty windows.")
        logger.debug(
            "TaxoNERD span-window: %d windows (max_chars=%d overlap=%d merge=%s)",
            len(windows), max_window_chars, overlap_chars, merge_spans,
        )
        ...
        logger.debug("TaxoNERD span-window: %d mentions extracted across %d windows", len(mentions), len(windows))

        logger.info(
            "TaxoNERD span-window linking %d mentions with strategy=%s linker=%s",
            len(mentions), linker_strategy, linker,
        )
        taxons, linker_metrics = TaxoNERDAnnotator._taxons_from_mentions(
            mentions,
        ...
        logger.info(
            "TaxoNERD span-window: %d taxons from %d mentions in %d windows",
            len(taxons), len(mentions), len(windows),
        )
        return StrategyResult(
            taxons,
            windows=len(windows),
    ...
    def _run_strategy(
        ...
        logger.debug("TaxoNERD dispatching strategy: %s", strategy)
        ...
        if strategy == "legacy-compatible":
            return TaxoNERDAnnotator._run_legacy_compatible_whole_document(
                text, model, linker, threshold, exclude, prefer_gpu,
                neighbours, ann_ef_search, linker_strategy, link_cache,
                sparql_endpoint, sparql_batch_size, sparql_concurrency, sparql_timeout,
            )
        if strategy == "span-window":
            return TaxoNERDAnnotator._run_span_windows(
                ...
                batch_size, n_process, neighbours, ann_ef_search, linker_strategy,
                link_cache, sparql_endpoint, sparql_batch_size, sparql_concurrency, sparql_timeout,
            )
        logger.error("TaxoNERD unhandled strategy: %s", strategy)
        raise ValueError(f"unhandled strategy: {strategy}")

    # ==================================================================
    # process() – the single entry point readers should study
    # ==================================================================

    async def process(self, doc: V1RequestEnvelope) -> AsyncIterator[object]:
        started = time()
        logger.info("TaxoNERD process started")

        # -- resolve parameters -------------------------------------------------
        text = sofa_text_value(doc.sofa) or ""
        parameters = doc.parameters
        logger.debug("TaxoNERD sofa text length: %d, fs count: %d", len(text), len(doc.fs))

        strategy = self._strategy(parameters.get("input_strategy") or parameters.get("strategy"))
        logger.info("TaxoNERD resolved strategy: %s", strategy)

        model = self._model_name(parameters.get("model") or parameters.get("model_name"))
        logger.info("TaxoNERD resolved model: %s", model)

        linker = self._linker_name(parameters.get("linking") or parameters.get("linker_name"))
        logger.info("TaxoNERD resolved linker: %s", linker)

        linker_strategy = self._linker_strategy(
            parameters.get("linker_strategy") or parameters.get("linking_strategy")
        )
        logger.info("TaxoNERD resolved linker_strategy: %s", linker_strategy)

        link_cache = self._bool(parameters.get("link_cache"), True)
        logger.debug("TaxoNERD link_cache: %s", link_cache)

        if linker is None and not self._bool(parameters.get("allow_unlinked"), False):
        ...
        prefer_gpu = self._bool(parameters.get("prefer_gpu"), False)
        logger.debug("TaxoNERD prefer_gpu: %s", prefer_gpu)

        threshold = self._float(parameters.get("threshold"), 0.7)
        logger.debug("TaxoNERD threshold: %.3f", threshold)

        exclude = self._exclude(parameters.get("exclude"))
        logger.debug("TaxoNERD exclude: %s", list(exclude))

        span_types = self._aliased_types(
            parameters.get("span_types"), DEFAULT_SPAN_PRIORITY, SPAN_TYPE_ALIASES,
        )
        logger.debug("TaxoNERD span_types: %s", list(span_types))

        max_window_chars = self._int(parameters.get("max_window_chars"), 2500, minimum=128)
        overlap_chars = self._int(parameters.get("overlap_chars"), 160, minimum=0)
        merge_spans = self._bool(parameters.get("merge_spans"), False)
        batch_size = self._int(parameters.get("batch_size"), 8, minimum=1)
        n_process = self._int(parameters.get("n_process"), 1, minimum=1)
        logger.debug(
            "TaxoNERD window params: max_chars=%d overlap=%d merge=%s batch=%d n_process=%d",
            max_window_chars, overlap_chars, merge_spans, batch_size, n_process,
        )

        neighbours = self._int(parameters.get("neighbours"), 10, minimum=1)
        ann_ef_search = self._int(parameters.get("ann_ef_search"), 80, minimum=10)
        logger.debug("TaxoNERD ANN params: neighbours=%d ef_search=%d", neighbours, ann_ef_search)

        sparql_endpoint = self._string(
            parameters.get("sparql_endpoint"),
            DEFAULT_SPARQL_ENDPOINT,
        )
        sparql_batch_size = self._int(parameters.get("sparql_batch_size"), 64, minimum=1)
        sparql_concurrency = self._int(parameters.get("sparql_concurrency"), 8, minimum=1)
        sparql_timeout = self._float(parameters.get("sparql_timeout"), 20.0)
        logger.debug(
            "TaxoNERD SPARQL params: endpoint=%s batch=%d concurrency=%d timeout=%.1f",
            sparql_endpoint if linker == "gbif_fuseki" else "(not used)",
            sparql_batch_size, sparql_concurrency, sparql_timeout,
        )

        # -- telemetry: request configured --------------------------------------
        self._emit_telemetry(telemetry.trace(
        ...
        async def run_taxonerd_strategy() -> StrategyResult:
            ...
            logger.info("TaxoNERD strategy execution starting: %s", strategy)
            try:
                ...
                logger.info("TaxoNERD strategy execution completed in %.1f ms", elapsed)
        ...
        try:
            ...
            logger.error(
                "TaxoNERD strategy failed: strategy=%s exception=%s detail=%s",
                strategy, type(exc).__name__, str(exc),
            )
        ...
        logger.info(
            "TaxoNERD processing completed in %d ms: %d taxons, %d windows, %d mentions",
            elapsed_ms, len(result.taxons), result.windows, result.mentions,
        )
        ...
        self._emit_telemetry(telemetry.count(
        ...
        self._emit_telemetry(telemetry.count("taxonerd_input_windows", result.windows, strategy=strategy))
        self._emit_telemetry(telemetry.count("taxonerd_input_mentions", result.mentions, strategy=strategy))
        ...
        for metric_name, metric_value in result.metrics.items():
            if metric_name.endswith("_ms"):
                self._emit_telemetry(
                    telemetry.timing(f"taxonerd_{metric_name}", metric_value, **metric_attrs)
                )
            else:
                self._emit_telemetry(
                    telemetry.gauge(
                        f"taxonerd_{metric_name}", float(metric_value), "count", **metric_attrs,
                    )
                )

        self._emit_telemetry(telemetry.debug(
        ...
        # -- build result -------------------------------------------------------
        logger.debug("TaxoNERD building annotation comments for %d taxons", len(result.taxons))
        ...
        logger.info("TaxoNERD process finished: yielded 1 DuuiResult")
...
def _preload_runtime() -> None:
    ...
    model = TaxoNERDAnnotator._model_name(None)
    linker_values = TaxoNERDAnnotator._csv("gbif_backbone", ("gbif_backbone",))
    linkers = tuple(
        linker
        for value in linker_values
        for linker in [TaxoNERDAnnotator._linker_name(value)]
        if linker is not None
    )
    exclude = TaxoNERDAnnotator._exclude(None)
    prefer_gpu = False
    ann_ef_search = 80
    threshold = 0.7

    logger.info("TaxoNERD preloading legacy components: model=%s", model)
    TaxoNERDAnnotator._load_taxonerd_legacy_components(model, exclude, prefer_gpu)

    for linker in linkers:
        logger.info("TaxoNERD preloading linker: %s", linker)
        TaxoNERDAnnotator._load_taxonerd(model, linker, threshold, exclude, prefer_gpu)
        TaxoNERDAnnotator._load_exact_linker(linker)
        TaxoNERDAnnotator._candidate_generator(linker, ann_ef_search)
...
async def preload_runtime_on_startup() -> None:
    ...
    try:
        ...
        logger.info("TaxoNERD runtime preloaded successfully")
    except Exception:
        print("TAXONERD PRELOAD FAILED:", file=sys.stderr)
        traceback.print_exc(file=sys.stderr)
        logger.error("TaxoNERD preload failed", exc_info=True)
