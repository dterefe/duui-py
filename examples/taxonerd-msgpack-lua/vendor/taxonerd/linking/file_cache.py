...
def _cached_file_for_url(url: str, cache_dir: str) -> Union[str, None]:
    """
    Return an already cached artifact for a URL without touching the network.

    The upstream cache implementation always performs a HEAD request to recover
    the current ETag before it looks for the file. That makes hot startup and
    process requests depend on taxonerd.texttechnologylab.org even when the
    artifact is already mounted in /root/.taxonerd. For benchmarked DUUI runs the
    local cache is the source of truth; a network revalidation belongs outside
    the request path.
    """
    last_part = url.split("/")[-1]
    metadata_suffix = f".{last_part}.json"
...
def get_from_cache(url: str, name: str, cache_dir: str = None) -> str:
    ...
    cached_path = _cached_file_for_url(url, cache_dir)
