...
from duui_py.models.uima_typesystem.texttechnologylab.annotation.semaf.isobase.types import (
    Entity,
)
from duui_py.models.uima_typesystem.texttechnologylab.annotation.semaf.semafsr.types import (
    SrLink,
)
...
class SRLAnnotator(DuuiAnnotator[V1RequestEnvelope, object]):
    ...
    async def process(self, doc: V1RequestEnvelope) -> AsyncIterator[object]:
        ...
        await telemetry.info(
            ...
            max_links_per_sentence=max_links,
        )
        tokens = sorted(
            [
                fs
                ...
                and self._is_type(fs.type, TOKEN_TYPE)
            ],
        ...
        sentences = sorted(
            [
                ...
                if fs.begin is not None
                and fs.end is not None
                and self._is_type(fs.type, SENTENCE_TYPE)
            ],
            key=lambda fs: (fs.begin or 0, fs.end or 0),
        )
        ...
        for sent in sentences:
            sent_token_ids = [
                ...
                if (tok.begin or 0) >= (sent.begin or 0)
                and (tok.end or 0) <= (sent.end or 0)
            ]
        ...
        await telemetry.count("srl_tokens", len(tokens))
        await telemetry.count("srl_sentences", len(sentences))
        await telemetry.count("srl_links", link_counter)
        await telemetry.count("srl_annotations", annotations)
        await telemetry.timing("srl_processing_ms", elapsed_ms)
        await telemetry.info(
